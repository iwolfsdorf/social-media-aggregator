...
public class BoardServiceImpl implements BoardService {
	...
	@Override
	public Board getByUserAndDescription(final User user, final String description) throws ServiceException {
		final Optional<Board> optionalBoard = boardDao.findByUserAndDescription(user, description);
		if (optionalBoard.isPresent()) {
			return optionalBoard.get();
		} else {
			throw new ServiceException("No existe el board con usuario " + user.getId() + " y descripcion " + description);
		}
	}
